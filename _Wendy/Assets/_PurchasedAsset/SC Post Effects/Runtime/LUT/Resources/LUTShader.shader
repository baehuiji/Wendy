Shader "Hidden/SC Post Effects/LUT"
{
	...
	float4 FragSingle(Varyings i) : SV_Target
	{
		...
		half3 colorGraded = Grade(TEX_PARAM(_LUT_Near, Clamp), screenColor.rgb);
	...
	float4 FragDuo(Varyings i) : SV_Target
	{
		...
		half3 gradedNear = Grade(TEX_PARAM(_LUT_Near, Clamp), screenColor.rgb);
		half3 gradedFar = Grade(TEX_PARAM(_LUT_Far, Clamp), screenColor.rgb);
